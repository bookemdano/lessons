...
std::list<IntPair> graphBFS(const IntPair& start, const IntPair& goal, const GridGraph& graph) {
  ...
  while (!exploreQ.empty() && !foundGoal && !tooManySteps) {
    ...
    GridGraph::NeighborSet neighbors = graph.adjacencyMap.at(curPoint); // ChangeD this...
    // =====================================================================
    ...
    for (auto neighbor : neighbors) {
      ...
      bool neighborWasAlreadyVisited = visitedSet.count(neighbor); // ChangeD this...
      // ==================================================================
...
// puzzleBFS:
// Given start and goal states as PuzzleState objects, we perform BFS in the
...
std::list<PuzzleState> puzzleBFS(const PuzzleState& start, const PuzzleState& goal) {
  ...
  while (!exploreQ.empty() && !foundGoal && !tooManySteps) {
    ...
    auto neighbors = curState.getAdjacentStates(); // ChangeD this! This line is totally wrong.
